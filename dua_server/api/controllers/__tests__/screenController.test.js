...
describe('Screen controller', () => {
  ...
  describe('createScreen', () => {
    ...
    it('should return a 500 if there is an error creating the screen', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ error: 'Error in screenController - createScreen: failed to make new screen' });
  ...
  describe('finishScreen', () => {
    it('should update screen and return updated screen JSON', async () => {
      // Arrange
      const req = { params: { uid: '1' }, body: { SCREEN_DURATION_SECONDS: 10, EXIT_METHOD: 'back', VIDEO_TIME_AT_END: 8 } };
      const res = { send: jest.fn() };
      const updatedScreen = { toJSON: jest.fn().mockReturnValueOnce({ UID: '1', SCREEN_DURATION_SECONDS: 10, EXIT_METHOD: 'back', VIDEO_TIME_AT_END: 8 }) };
      ...
      expect(res.send).toHaveBeenCalledWith({ UID: '1', SCREEN_DURATION_SECONDS: 10, EXIT_METHOD: 'back', VIDEO_TIME_AT_END: 8 });
    });

    it('should return 404 error when screen is not found', async () => {
      // Arrange
      const req = { params: { uid: '2' }, body: { SCREEN_DURATION_SECONDS: 10, EXIT_METHOD: 'back', VIDEO_TIME_AT_END: 8 } };
      ...
      expect(res.send).toHaveBeenCalledWith({ error: 'ScreenController - finishScreen: Screen not found' });
    });
    ...
    it('should return 500 error when there is an error in update', async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.send).toHaveBeenCalledWith({ error: 'Error in screenController - finishScreen' });
    });
  });
  ...
  // Get Screen
    ...
    it('should return screen if found', async () => {
        ...
        const res = { send: jest.fn() };
        const screen = { toJSON: jest.fn().mockReturnValueOnce({ UID: '1', SCREEN_DURATION_SECONDS: 10, EXIT_METHOD: 'exit', VIDEO_TIME_AT_END: 5 }) };
        ...
        // Assert
        expect(res.send).toHaveBeenCalledWith(expect.objectContaining({ UID: '1', SCREEN_DURATION_SECONDS: 10, EXIT_METHOD: 'exit', VIDEO_TIME_AT_END: 5 }));
    });
    ...
    it('should return 404 if screen not found', async () => {
        ...
        // Assert
        expect(res.status).toHaveBeenCalledWith(404);
        expect(res.send).toHaveBeenCalledWith({ error: 'ScreenController - getScreen: Screen not found' });
    });
    ...
    it('should return 500 if an error occurs', async () => {
        ...
        const req = { params: { uid: '1' } };
        const res = { status: jest.fn().mockReturnThis(), send: jest.fn() };
        jest.spyOn(Screen, 'findOne').mockRejectedValueOnce(new Error('Database error'));
        ...
        // Assert
        expect(res.status).toHaveBeenCalledWith(500);
        expect(res.send).toHaveBeenCalledWith({ error: 'Error in screenController - getScreen' });
    });
    });
});